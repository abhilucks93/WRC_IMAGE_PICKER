...
public class PreviewFragment extends Fragment{
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mImageList = wrcImagePicker.getImageItemsOfCurrentImageSet();
        mCurrentItemPosition = getArguments().getInt(WrcImagePicker.KEY_PIC_SELECTED_POSITION,0);
    ...
    private void initView(View contentView) {
        ...
        if(mContext instanceof OnImagePageSelectedListener){
            ...
            if(wrcImagePicker.isSelect(mCurrentItemPosition,item) ){
                isSelected = true;
            }
    ...
    public void selectCurrent(boolean isCheck){
        ImageItemBean item = mImageList.get(mCurrentItemPosition);
        boolean isSelect = wrcImagePicker.isSelect(mCurrentItemPosition,item);
        if(isCheck){
            if(!isSelect){
                WrcImagePicker.getInstance().addSelectedImageItem(mCurrentItemPosition,item);
            }
        }else{
            if(isSelect){
                WrcImagePicker.getInstance().deleteSelectedImageItem(mCurrentItemPosition, item);
            }
        }
