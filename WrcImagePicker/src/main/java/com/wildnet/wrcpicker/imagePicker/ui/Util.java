...
public class Util {
    ...
    public static boolean IdExist(int id, Activity act) {
        String name = null;
        try {
            name = act.getApplicationContext().getResources().getResourceName(id);
        } catch (Exception e) {
            return false;
        }
        if (name == null || !name.startsWith(act.getApplication().getPackageName())) {
            return false;
        }
        return true;
    }
